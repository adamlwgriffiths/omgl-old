...
class TestTexture3D(unittest.TestCase):
    ...
    def texture_auto(self, size, dtype):
        data = np.arange(32*32*32*size, dtype=dtype)
        data.shape = (32,32,32,size)
        t = omgl.texture3d.create(data)
        actual = t.get_data()
    ...
    def test_texture_auto_invalid(self):
        data = np.arange(32*32*32*4, dtype=np.int16)
        with self.assertRaises(ValueError):
            t = omgl.texture3d.create(data)

    def test_default_min_filter(self):
        data = np.arange(32*32*32*4, dtype=np.float32)
        data.shape = (32,32,32,4)
        t = omgl.texture3d.create(data, min_filter=GL.GL_NEAREST)
    ...
    def test_default_mag_filter(self):
        data = np.arange(32*32*32*4, dtype=np.float32)
        data.shape = (32,32,32,4)
        t = omgl.texture3d.create(data, mag_filter=GL.GL_NEAREST)
    ...
    def test_default_wrap_s(self):
        data = np.arange(32*32*32*4, dtype=np.float32)
        data.shape = (32,32,32,4)
        t = omgl.texture3d.create(data, wrap_s=GL.GL_CLAMP_TO_EDGE)
    ...
    def test_default_wrap_t(self):
        data = np.arange(32*32*32*4, dtype=np.float32)
        data.shape = (32,32,32,4)
        t = omgl.texture3d.create(data, wrap_t=GL.GL_CLAMP_TO_EDGE)
    ...
    def test_default_wrap_r(self):
        data = np.arange(32*32*32*4, dtype=np.float32)
        data.shape = (32,32,32,4)
        t = omgl.texture3d.create(data, wrap_r=GL.GL_CLAMP_TO_EDGE)
        self.assertEqual(t.wrap_r, GL.GL_CLAMP_TO_EDGE)
        with t:
            filter = GL.glGetTexParameteriv(t.target, GL.GL_TEXTURE_WRAP_R)
            self.assertEqual(t.wrap_r, filter)
